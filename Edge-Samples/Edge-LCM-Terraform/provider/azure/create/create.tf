...
resource "null_resource" "azcopy_copy" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      azcopy copy ${module.admin_create.vhd_name} "https://${data.azurerm_storage_account.storage.name}.blob.core.windows.net/${data.azurerm_storage_container.container.name}${data.azurerm_storage_account_sas.token.sas}" --recursive=true
    EOT
  }
}
